...
JValidationErrorBox::JValidationErrorBox(JNIEnv * env, jobject jerror) {
	...
	if( this->jreported_by != NULL ) {
		this->error->ReportedBy = (char*)env->GetStringUTFChars(this->jreported_by, &isCopy);
	}

	if( this->jtype != NULL ) {
		this->error->Type = (char*)env->GetStringUTFChars(this->jtype, &isCopy);
	}

	this->error->Code = this->jcode;

	if( this->jtitle != NULL ) {
		this->error->Title = (char*)env->GetStringUTFChars(this->jtitle, &isCopy);
	}

	if( this->jdetail != NULL ) {
		this->error->Detail = (char*)env->GetStringUTFChars(this->jdetail, &isCopy);
	}
	...
	if( this->jposition != NULL ) {
		...
		if( this->jfilepath != NULL ) {
			this->error->Position->Filepath = (char*)env->GetStringUTFChars(this->jfilepath, &isCopy);
		}
	...
	if( this->jtrace != NULL ) {
		...
		for(int i = 0; i < trace_size; i++ ) {
			...
			this->error->Trace[i] = (char *) env->GetStringUTFChars(this->jtrace_items[i], &isCopy);
		}
...
JValidationErrorBox::~JValidationErrorBox() {
    ...
    if(this->error != NULL) {
        if(this->error->ReportedBy != NULL) {
            this->env->ReleaseStringUTFChars(this->jreported_by, this->error->ReportedBy);
        }

        if(this->error->Type != NULL) {
            this->env->ReleaseStringUTFChars(this->jtype, this->error->Type);
        }

        if(this->error->Title != NULL) {
            this->env->ReleaseStringUTFChars(this->jtitle, this->error->Title);
        }

        if(this->error->Detail != NULL) {
            this->env->ReleaseStringUTFChars(this->jdetail, this->error->Detail);
        }
        ...
        if( this->error->Position != NULL ) {
            ...
            if(this->error->Position->Filepath != NULL) {
                this->env->ReleaseStringUTFChars(this->jfilepath, this->error->Position->Filepath);
            }
        ...
        if( this->error->Trace != NULL ) {
            for(int i = 0; i < this->trace_size; i++) {
                this->env->ReleaseStringUTFChars(this->jtrace_items[i], this->error->Trace[i]);
            }
